...
///
/// IDs generated by [`CorrelationId::new`] are UUID v4 strings for global uniqueness.
/// When an upstream trace context is available (e.g. a W3C `traceparent` header), the
/// upstream value is preserved verbatim via `From<TraceContext>` rather than generating
/// a new UUID.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
impl CorrelationId {
    ...
    ///
    /// The generated ID uses UUID v4 format. Upstream-supplied trace identifiers are
    /// preserved verbatim via `From<TraceContext>`.
    pub fn new() -> Self {
...
/// Trace context extracted from incoming HTTP headers.
///
/// Captures the upstream trace identifier from one of three recognised headers,
...
///
/// The raw header value is preserved verbatim and can be converted to a
/// [`CorrelationId`] via `From<TraceContext>`, threading the upstream trace identity
/// through to queue messages and audit log entries.
///
/// # Examples
///
/// ```rust
/// use std::collections::HashMap;
/// use queue_keeper_core::TraceContext;
///
...
impl TraceContext {
    ...
    /// use queue_keeper_core::TraceContext;
    ///
    /// // traceparent takes priority over x-correlation-id
    /// let mut headers = HashMap::new();
    /// headers.insert("traceparent".to_string(), "00-abc-def-01".to_string());
    ...
    /// Get the raw trace context string.
    pub fn as_str(&self) -> &str {
        &self.0
    }
}
...
impl From<TraceContext> for CorrelationId {
    /// Convert a [`TraceContext`] into a [`CorrelationId`], preserving the upstream
    /// trace value verbatim.
